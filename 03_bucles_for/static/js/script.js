...
function reordenarDatos() {
    ...
    resultado.textContent = `hubieron ${contadorAprobados} alumnos aprobados`
    container.classList.remove(`d-none`)
}
...
Recorre el arreglo. Si el índice actual (i) es par (puedes usar i % 2 === 0), agrega la palabra
" [VIP]" al lado de su nombre y muéstralo en el párrafo. Si es impar, muéstralo normal.
*/
...
function traducirAcciones() {
    ...
    for (let i = 0; i < asistentes.length; i++) {
        if (i % 2 === 0) {
            resultado1.push(`${asistentes[i]} [VIP]`);
        } else {
            resultado1.push(`${asistentes[i]}`);
        }
    }
    resultado.textContent = resultado1.join(" - ")
    container.classList.remove(`d-none`);
}
...
function calificacionChile() {
    ...
    resultado.textContent = `El articulo ${articuloBuscado} se encuentra ${vecesEncontrado} veces en la bodega`
    container.classList.remove(`d-none`);
}
...
function invitados() {
    ...
    for (let i = 0; i < temperaturas.length; i++) {
        if (temperaturas[i] >= 30 && temperaturas[i] <= 50) {
            resultado.innerHTML += `<p class = "text-danger"> "¡ALERTA! Temperatura crítica de " ${temperaturas[i]} " grados. "</p> <br>`
        }
    }
    container.classList.remove(`d-none`);
}
